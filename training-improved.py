...
def create_model(filters_1=32, filters_2=32, filters_3=64, 
    ...
    model.add(Conv2D(filters_1, (3, 3), input_shape=input_shape))
    ...
    model.add(Conv2D(filters_3, (3, 3)))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
...
final_model.save('model-rs.keras')
final_model.save_weights('model-rs.weights.h5')
...
with open('history-rs.json', 'w') as f:
...
plt.tight_layout()
plt.savefig(os.path.join(vis_dir, 'model_training_history.png'))
